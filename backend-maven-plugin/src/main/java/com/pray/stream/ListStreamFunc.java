...
public class ListStreamFunc {
    ...
    private static final List<String> finalList = List.of(
            "小明","王大奶","Johnny","蜜雪冰城","345","霸王茶姬","Star Bulks","王大奶","霸王茶姬","321"
    );
...
//        }
        ...
        System.out.println(finalList.stream().distinct().toList());

        //排序：sorted(),指定排序比较器
        System.out.println(finalList.stream().sorted(
                (o1,o2) -> o1.length()-o2.length()
        ).toList());

        //限制流的大小或者跳过一些元素：limited(),skip()
        System.out.println(finalList.stream().limit(4).skip(2).toList());

        //终结操作，指的是将流终结或者收集起来组装成数据
        //forEach(fun)

        //组装成Set、List
        System.out.println(new HashSet<>(finalList).stream().toList());
